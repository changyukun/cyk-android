...
struct OMXCodec : public MediaSource, public MediaBufferObserver 
{
	...
	static sp<MediaSource> Create(const sp<IOMX> &omx,
	...
	static void setComponentRole( const sp<IOMX> &omx, IOMX::node_id node, bool isEncoder, const char *mime);
	...
	// for use by ACodec
	static void findMatchingCodecs(const char *mime,
									bool createEncoder, 
...
private:
	...
	status_t setVideoPortFormatType(OMX_U32 portIndex,
	OMX_VIDEO_CODINGTYPE compressionFormat,
	OMX_COLOR_FORMATTYPE colorFormat);

	void setVideoInputFormat(const char *mime, const sp<MetaData>& meta);
	...
	status_t isColorFormatSupported(OMX_COLOR_FORMATTYPE colorFormat, int portIndex);
	...
	status_t setVideoOutputFormat(const char *mime, OMX_U32 width, OMX_U32 height);

	void setImageOutputFormat(OMX_COLOR_FORMATTYPE format, OMX_U32 width, OMX_U32 height);

	void setJPEGInputFormat(OMX_U32 width, OMX_U32 height, OMX_U32 compressedSize);
	...
	status_t freeBuffersOnPort(OMX_U32 portIndex, bool onlyThoseWeOwn = false);
	...
	static uint32_t getComponentQuirks(const char *componentName, bool isEncoder);
	...
	status_t parseAVCCodecSpecificData(const void *data, size_t size, unsigned *profile, unsigned *level);
...
status_t QueryCodecs(const sp<IOMX> &omx,
