...
TimedEventQueue::TimedEventQueue() : mNextEventID(1), mRunning(false), mStopped(false) 
{
/*
	参数:
		1、
...
/*
	...
	说明:
		1、实质就是将传入的事件event  插入到容器队列中( 将事件插入到队列头)
*/
...
TimedEventQueue::event_id TimedEventQueue::postEventToBack(const sp<Event> &event) 
{
/*
	参数:
		1、
	...
	说明:
		1、实质就是将传入的事件event  插入到容器队列中( 将事件插入到队列尾)
*/
	return postTimedEvent(event, INT64_MAX);
}

TimedEventQueue::event_id TimedEventQueue::postEventWithDelay(const sp<Event> &event, int64_t delay_us) 
{
/*
	参数:
		1、
	...
	说明:
		1、实质就是将传入的事件event  插入到容器队列中( 以当前时间加上延时的总时间值作为排序插入到队列中)
*/
	CHECK(delay_us >= 0);
	return postTimedEvent(event, getRealTimeUs() + delay_us);
}

TimedEventQueue::event_id TimedEventQueue::postTimedEvent(const sp<Event> &event, int64_t realtime_us) 
{
/*
	参数:
		1、event		: 传入一个待插入到队列的事件
		2、realtime_us	: 传入一个事件对应的事件值，此值为时间，事件就是根据这个时间值进行排序的
		
	返回:
		1、
		
	说明:
		1、实质就是将传入的事件event  插入到容器队列中
		2、插入队列中的各个事件的排序是按照事件的时间值来进行排序的，即参数realtime_us
*/
...
static bool MatchesEventID(void *cookie, const sp<TimedEventQueue::Event> &event) 
{
/*
	参数:
		1、
	...
	说明:
		1、
*/
	TimedEventQueue::event_id *id = static_cast<TimedEventQueue::event_id *>(cookie);
...
void TimedEventQueue::cancelEvents(	bool (*predicate)(void *cookie, const sp<Event> &event),
									void *cookie,
...
/*
	...
	说明:
		1、此线程的创建见函数AwesomePlayer::prepareAsync_l()  的start 调用
*/
	...
	for (;;)
	{
...

		{
			...
			while (mQueue.empty()) /* 事件队列为空，则等待事件的到来*/
			{
			...
			for (;;) 
			{
				...
				now_us = getRealTimeUs(); /* 取出当前时间*/
				int64_t when_us = (*it).realtime_us; /* 取出事件要被处理的时间*/
				...
				status_t err = mQueueHeadChangedCondition.waitRelative(mLock, delay_us * 1000ll);
...
sp<TimedEventQueue::Event> TimedEventQueue::removeEventFromQueue_l(event_id id) 
{
/*
	参数:
		1、
