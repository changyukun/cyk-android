...
status_t AwesomePlayer::setDataSource_l( const char *uri, const KeyedVector<String8, String8> *headers) 
{
/*
	参数:
		1、
	...
	for (size_t i = 0; i < extractor->countTracks(); ++i) 
	{
		...
		else if (!haveAudio && !strncasecmp(mime.string(), "audio/", 6))
		{
			...
			if (!strcasecmp(mime.string(), MEDIA_MIMETYPE_AUDIO_VORBIS)) 
			{
				...
				int32_t loop;
				if (fileMeta != NULL&& fileMeta->findInt32(kKeyAutoLoop, &loop) && loop != 0) 
				{
...
*/
	...
	if ((mAudioPlayer == NULL || !(mFlags & AUDIOPLAYER_STARTED))&& mAudioSource != NULL) 
	{
...
*/
	off64_t size;
	if (mDurationUs >= 0 && mCachedSource != NULL&& mCachedSource->getSize(&size) == OK) 
	{
...
*/
	...
	if (!(mFlags & VIDEO_AT_EOS) && videoLateByUs > 300000ll) 
	{
		...
		notifyListener_l (MEDIA_INFO, MEDIA_INFO_VIDEO_TRACK_LAGGING, videoLateByUs / 1000ll);
	}
...
*/
	...
	if (mCachedSource != NULL) 
	{
		...
		else
		{
			...
			else
			{
				...
				if ((mFlags & PLAYING) && !eos && (cachedDataRemaining < kLowWaterMarkBytes))
				{
					ALOGI("cache is running low (< %d) , pausing.",kLowWaterMarkBytes);
				...
				else if (eos || cachedDataRemaining > kHighWaterMarkBytes) 
				{
					...
					{
						ALOGI("cache has filled up (> %d), resuming.", kHighWaterMarkBytes);
					...
					{
						ALOGV("cache has filled up (> %d), prepare is done", kHighWaterMarkBytes);
						finishAsyncPrepare_l();
					}
				}
			}
	...
	if (getCachedDuration_l(&cachedDurationUs, &eos)) 
	{
		...
		if ((mFlags & PLAYING) && !eos && (cachedDurationUs < kLowWaterMarkUs))
		{
			ALOGI("cache is running low (%.2f secs) , pausing.", cachedDurationUs / 1E6);
			modifyFlags(CACHE_UNDERRUN, SET);
			pause_l();
			ensureCacheIsFetching_l();
			sendCacheStats();
		...
		else if (eos || cachedDurationUs > kHighWaterMarkUs) 
		{
			if (mFlags & CACHE_UNDERRUN) 
			{
				ALOGI("cache has filled up (%.2f secs), resuming.", cachedDurationUs / 1E6);
				modifyFlags(CACHE_UNDERRUN, CLEAR);
				play_l();
				notifyListener_l(MEDIA_INFO, MEDIA_INFO_BUFFERING_END);
			} 
			...
			{
				ALOGV("cache has filled up (%.2f secs), prepare is done",cachedDurationUs / 1E6);
				finishAsyncPrepare_l();
			}
		}
	}
...
*/
	...
	if (listener != NULL && mCachedSource != NULL) 
	{
		...
		if (err == OK)
		{
			listener->sendEvent(MEDIA_INFO, MEDIA_INFO_NETWORK_BANDWIDTH, kbps);
		}
	}
}
...
*/
	...
	const bool allDone = (mVideoSource == NULL || (mFlags & VIDEO_AT_EOS)) && (mAudioSource == NULL || (mFlags & AUDIO_AT_EOS));
	...
	if ((mFlags & LOOPING)|| ((mFlags & AUTO_LOOPING)&& (mAudioSink == NULL || mAudioSink->realtime()))) 
	{
...
*/
	...
	if (mDecryptHandle != NULL) 
	{
		...
		mDrmManagerClient->setPlaybackStatus(mDecryptHandle,Playback::START, position / 1000);
	}
	...
	uint32_t params = IMediaPlayerService::kBatteryDataCodecStarted  | IMediaPlayerService::kBatteryDataTrackDecoder;
	if ((mAudioSource != NULL) && (mAudioSource != mAudioTrack)) 
	{
		params |= IMediaPlayerService::kBatteryDataTrackAudio;
	}
...
*/
	...
	{
		ALOGV("got crop rect %d, %d, %d, %d",cropLeft, cropTop, cropRight, cropBottom);
	}
	...
	{
		notifyListener_l(MEDIA_SET_VIDEO_SIZE, usableHeight, usableWidth);
	}
	else
	{
		notifyListener_l(MEDIA_SET_VIDEO_SIZE, usableWidth, usableHeight);
	}
}
...
*/
	...
	if (mDecryptHandle != NULL) 
	{
		mDrmManagerClient->setPlaybackStatus(mDecryptHandle, Playback::PAUSE, 0);
	}
...
*/
	if (mTextPlayer != NULL) 
	{
		...
		{ // to turn off the text track display
...
*/
	...
	if (!(mFlags & PLAYING))
	{
		ALOGV("seeking while paused, sending SEEK_COMPLETE notification" " immediately.");
...
*/
	if (mSeeking != NO_SEEK && mVideoSource == NULL && mAudioPlayer != NULL)
	{
		...
		{
			mDrmManagerClient->setPlaybackStatus(mDecryptHandle, Playback::PAUSE, 0);
			mDrmManagerClient->setPlaybackStatus(mDecryptHandle, Playback::START, mSeekTimeUs / 1000);
		}
	}
}
...
*/
	...
	if (mAudioSource != NULL) 
	{
		...
		const char *component;
		if (!mAudioSource->getFormat() ->findCString(kKeyDecoderComponent, &component)) 
		{
			component = "none";
		}
...
*/
	...
	if (mVideoSource != NULL) 
	{
		...
		CHECK(mVideoSource->getFormat() ->findCString(kKeyDecoderComponent, &componentName));
		...
		if (!strncmp(componentName, kPrefix, strlen(kPrefix))
						...
						&& !strcmp(&componentName[componentNameLength - kSuffixLength], kSuffix)) 
		{
...
*/
	...
	{
		mDrmManagerClient->setPlaybackStatus(mDecryptHandle,Playback::PAUSE, 0);
		mDrmManagerClient->setPlaybackStatus(mDecryptHandle, Playback::START, videoTimeUs / 1000);
	}
}
...
*/
	...
	if (mSeeking != NO_SEEK) 
	{
		...
		if (mSeeking == SEEK && isStreamingHTTP() && mAudioSource != NULL&& !(mFlags & SEEK_PREVIEW))
		{
	...
	if (mSeeking == SEEK_VIDEO_ONLY) 
	{
		...
		{
			ALOGI("XXX mSeekTimeUs = %lld us, timeUs = %lld us",mSeekTimeUs, timeUs);
		}
	}
	...
	if (!(mFlags & AUDIO_AT_EOS) && mAudioPlayer != NULL && mAudioPlayer->getMediaTimeMapping(&realTimeUs, &mediaTimeUs)) 
	{
	...
	if (wasSeeking == NO_SEEK)
	{
		...
		{
			ALOGI("we're much too late (%.2f secs), video skipping ahead", latenessUs / 1E6);
		...
		if (latenessUs > 40000) 
		{
			...
			ALOGV("we're late by %lld us (%.2f secs)",latenessUs, latenessUs / 1E6);

			if (!(mFlags & SLOW_DECODER_HACK)|| mSinceLastDropped > FRAME_DROP_FREQ)
			{
				ALOGV("we're late by %lld us (%.2f secs) dropping "  "one after %d frames", latenessUs, latenessUs / 1E6, mSinceLastDropped);
	...
	if ((mNativeWindow != NULL) && (mVideoRendererIsPreview || mVideoRenderer == NULL)) 
	{
		mVideoRendererIsPreview = false;
...
*/
	...
	if (!strncasecmp("http://", mUri.string(), 7) || !strncasecmp("https://", mUri.string(), 8) || isWidevineStreaming) 
	{
		...
		NuCachedSource2::RemoveCacheSpecificHeaders(&mUriHeaders, &cacheConfig, &disconnectAtHighwatermark);
...
#if 0
			mCachedSource = new NuCachedSource2(	new ThrottledSource(
		...
		if (strncasecmp(contentType.string(), "audio/", 6)) 
		{
			...
			if (mCachedSource != NULL) 
			{
				...
				for (;;)
				{
					...
					if (finalStatus != OK || (metaDataSize >= 0&& cachedDataRemaining >= metaDataSize)|| (mFlags & PREPARE_CANCELLED)) 
					{
						break;
					}
					...
					if (metaDataSize < 0 && cachedDataRemaining >= kMinBytesForSniffing) 
					{
						String8 tmp;
						float confidence;
						sp<AMessage> meta;
						...
						if (meta == NULL || !meta->findInt64( "meta-data-size", &metaDataSize)) 
						{
...
*/
	if (mCachedSource != NULL)
	{
		...
		ALOGD("Request to keep cache stats in the past %d ms", freqMs);
		...
		case KEY_PARAMETER_AUDIO_CHANNEL_COUNT:
			{
				...
				if (mAudioTrack == 0 || !mAudioTrack->getFormat()->findInt32(kKeyChannelCount, &channelCount))
				{
					channelCount = 0;
				}
...
*/
	...
	for (size_t i = 0; i < mStats.mTracks.size(); ++i) 
	{
		...
		{
			fprintf(out, "   videoDimensions(%d x %d), "
