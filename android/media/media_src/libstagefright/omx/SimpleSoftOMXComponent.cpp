...
OMX_ERRORTYPE SimpleSoftOMXComponent::sendCommand(OMX_COMMANDTYPE cmd, OMX_U32 param, OMX_PTR data) 
{
/*
	参数:
		1、
...
bool SimpleSoftOMXComponent::isSetParameterAllowed( OMX_INDEXTYPE index, const OMX_PTR params) const 
{
/*
	参数:
		1、
...
OMX_ERRORTYPE SimpleSoftOMXComponent::getParameter(OMX_INDEXTYPE index, OMX_PTR params) 
{
/*
	参数:
		1、
...
OMX_ERRORTYPE SimpleSoftOMXComponent::setParameter(OMX_INDEXTYPE index, const OMX_PTR params)
{
/*
	参数:
		1、
...
OMX_ERRORTYPE SimpleSoftOMXComponent::internalGetParameter(OMX_INDEXTYPE index, OMX_PTR params)
{
/*
	参数:
		1、
		...
		case OMX_IndexParamPortDefinition:
			{
				...
				const PortInfo *port = &mPorts.itemAt(defParams->nPortIndex);
...
OMX_ERRORTYPE SimpleSoftOMXComponent::internalSetParameter(OMX_INDEXTYPE index, const OMX_PTR params) 
{
/*
	参数:
		1、
		...
		case OMX_IndexParamPortDefinition:
			{
				...
				PortInfo *port = &mPorts.editItemAt(defParams->nPortIndex);
				...
				if (defParams->nBufferCountActual != port->mDef.nBufferCountActual) 
				{
					CHECK_GE(defParams->nBufferCountActual,port->mDef.nBufferCountMin);
...
OMX_ERRORTYPE SimpleSoftOMXComponent::useBuffer(	OMX_BUFFERHEADERTYPE **header,
													OMX_U32 portIndex,
													OMX_PTR appPrivate,
													OMX_U32 size,
													OMX_U8 *ptr) 
...
*/
	...
	*header = new OMX_BUFFERHEADERTYPE;
	...
	BufferInfo *buffer = &port->mBuffers.editItemAt(port->mBuffers.size() - 1);
...
OMX_ERRORTYPE SimpleSoftOMXComponent::allocateBuffer(	OMX_BUFFERHEADERTYPE **header,
														OMX_U32 portIndex,
														OMX_PTR appPrivate,
														OMX_U32 size) 
{
...
*/
	...
	OMX_ERRORTYPE err = useBuffer(header, portIndex, appPrivate, size, ptr);
...
OMX_ERRORTYPE SimpleSoftOMXComponent::freeBuffer(OMX_U32 portIndex,OMX_BUFFERHEADERTYPE *header) 
{
/*
	参数:
		1、
...
#if 0 // XXX
	CHECK((mState == OMX_StateIdle && mTargetState == OMX_StateLoaded) || port->mDef.bEnabled == OMX_FALSE);
#endif
...
OMX_ERRORTYPE SimpleSoftOMXComponent::emptyThisBuffer(OMX_BUFFERHEADERTYPE *buffer)
{
/*
	参数:
		1、
...
OMX_ERRORTYPE SimpleSoftOMXComponent::fillThisBuffer(OMX_BUFFERHEADERTYPE *buffer) 
{
/*
	参数:
		1、
				...
				for (size_t i = 0; i < mPorts.size(); ++i) 
				{
					...
					for (size_t j = 0; j < port->mBuffers.size(); ++j)
					{
						...
						if (buffer->mHeader == header)
						{
							...
							CHECK((msg->what() == kWhatEmptyThisBuffer && port->mDef.eDir == OMX_DirInput)|| (port->mDef.eDir == OMX_DirOutput));
...
void SimpleSoftOMXComponent::onSendCommand(OMX_COMMANDTYPE cmd, OMX_U32 param)
{
/*
	参数:
		1、
...
void SimpleSoftOMXComponent::onPortFlush( OMX_U32 portIndex, bool sendFlushComplete) 
{
/*
	参数:
		1、
...
void SimpleSoftOMXComponent::onPortEnableCompleted(OMX_U32 portIndex, bool enabled) 
{
/*
	参数:
		1、
...
SimpleSoftOMXComponent::PortInfo *SimpleSoftOMXComponent::editPortInfo(OMX_U32 portIndex) 
{
/*
	参数:
		1、
