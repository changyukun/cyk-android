...
SoftwareRenderer::SoftwareRenderer( const sp<ANativeWindow> &nativeWindow, const sp<MetaData> &meta)		: mConverter(NULL),
																										mYUVMode(None),
...
*/
	...
	if (!meta->findRect(kKeyCropRect,&mCropLeft, &mCropTop, &mCropRight, &mCropBottom)) 
	{
...
void SoftwareRenderer::render( const void *data, size_t size, void *platformPrivate)
{
/*
	参数:
		1、
...
*/
	...
	else
	{
		...
		const uint8_t *src_uv = (const uint8_t *)data + mWidth * (mHeight - mCropTop / 2);
