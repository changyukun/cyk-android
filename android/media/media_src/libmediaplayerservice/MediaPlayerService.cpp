...
*/
	...
	{
		status_t err = native_window_api_disconnect(mConnectedWindow.get(), NATIVE_WINDOW_API_MEDIA);
...
status_t MediaPlayerService::Client::setVideoSurfaceTexture(const sp<ISurfaceTexture>& surfaceTexture)
{
/*
	参数:
		1、
...
status_t MediaPlayerService::Client::invoke(const Parcel& request,Parcel *reply)
{
/*
	参数:
		1、
...
*/
	...
	if (unmarshallFilter(filter, &allow, &status) &&unmarshallFilter(filter, &drop, &status)) 
	{
		Mutex::Autolock lock(mLock);
...
status_t MediaPlayerService::Client::getMetadata( bool update_only, bool apply_filter, Parcel *reply)
{
/*
	参数:
		1、
