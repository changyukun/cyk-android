...
/*
	...
	说明:
		1、此函数为BnMediaPlayer  类的onTransact()  方法，因此此函数中调用的方法应该为BnMediaPlayer  子类
			的方法，见Client  类的定义就是继承了此类，因此此函数中调用的方法应该为Client  类的
			方法，如disconnect()  方法就应该为MediaPlayerService::Client::disconnect()
*/
	switch(code)
	{
		case DISCONNECT: 
		...
		case SET_PARAMETER:
			{
				...
				if (data.dataAvail() > 0)
				{
					request.appendFrom(const_cast<Parcel *>(&data), data.dataPosition(), data.dataAvail());
				}
