...
*/
	...
	IPCThreadState::self()->incWeakHandle(handle); /* 见类IPCThreadState  的各个方法*/
}
...
*/
	if (isDescriptorCached() == false) 
	{
		...
		status_t err = const_cast<BpBinder*>(this)->transact(INTERFACE_TRANSACTION, send, &reply);
		if (err == NO_ERROR) 
		{
...
*/
	...
	for (size_t i=0; i<N; i++)
	{
		...
		if ((obit.recipient == recipient|| (recipient == NULL && obit.cookie == cookie)) && obit.flags == flags) 
		{
...
*/
	...
	{
		if (ipc) 
			ipc->clearDeathNotification(mHandle, this);
		mObituaries = NULL;
	}
	mLock.unlock();
